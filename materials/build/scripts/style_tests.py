...
devs21_NULL_stderr = sys.stdout
devs21_NULL_stdout = sys.stdout
...
def copy_config():
    subprocess.run(['cp', PATH_TO_PROJECT + '/tests/.clang-format', PATH_TO_PROJECT], stdout=devs21_NULL_stdout,
                   stderr=devs21_NULL_stderr)


def delete_config():
    subprocess.run(['rm', PATH_TO_PROJECT + '/.clang-format'], stdout=devs21_NULL_stdout, stderr=devs21_NULL_stderr)
